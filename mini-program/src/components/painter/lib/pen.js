...
export default class Painter {
  ...
  _drawAbsImage(view) {
    ...
    this.ctx.save();
    const { width, height } = this._preProcess(view);
    /** object-fit: cover + panX/panY：铺满可视区域，可平移查看被裁切部分 */
    if (view.css && view.css.mode === 'aspectCover' && view.sWidth && view.sHeight) {
      const sw = view.sWidth;
      const sh = view.sHeight;
      const sc = Math.max(width / sw, height / sh);
      const dw = sw * sc;
      const dh = sh * sc;
      const panXStr = view.css.panX != null ? String(view.css.panX) : '0px';
      const panYStr = view.css.panY != null ? String(view.css.panY) : '0px';
      const px = panXStr.toPx ? panXStr.toPx() : 0;
      const py = panYStr.toPx ? panYStr.toPx() : 0;
      this.ctx.save();
      if (view.css.borderRadius) {
        this._doClip(view.css.borderRadius, width, height, view.css.borderStyle);
      } else {
        this.ctx.beginPath();
      ...
      this.ctx.drawImage(view.url, -(dw / 2) + px, -(dh / 2) + py, dw, dh);
      this.ctx.restore();
      this.ctx.restore();
      this._doBorder(view, width, height);
      return;
    }
    /** object-fit: contain + 可配 panX/panY（px 单位字符串）用于海报背景平移 */
    if (view.css && view.css.mode === 'aspectFit' && view.sWidth && view.sHeight) {
      const sw = view.sWidth;
      const sh = view.sHeight;
