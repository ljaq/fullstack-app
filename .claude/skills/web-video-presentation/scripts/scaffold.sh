...
#
# 用法：
#   bash scripts/scaffold.sh <target-dir> [--theme=<id>] [--aspect=16:9|9:16]
#   bash scripts/scaffold.sh <target-dir> [--platform=channels]  # 9:16 竖屏 alias
...
#   bash <path-to-web-video-presentation>/scripts/scaffold.sh ./channels --aspect=9:16 --theme=luji-trail
#   bash <path-to-web-video-presentation>/scripts/scaffold.sh ./channels --platform=channels
#
...
write_stage_config() {
  ...
  case "$aspect" in
    16:9)
      w=1920; h=1080; orientation="landscape"
      ;;
    9:16)
      w=1080; h=1920; orientation="portrait"
      ;;
    *)
      echo "✗ 不支持的画幅 '${aspect}'。可用：16:9、9:16" >&2
      exit 1
      ;;
  esac
  mkdir -p "$target_dir/src/config"
  cat > "$target_dir/src/config/stage.ts" <<EOF
/**
 * Stage dimensions — generated by scaffold.sh (--aspect / --platform).
 * See references/PLATFORMS.md for platform ↔ aspect mapping.
 */
export const STAGE = {
  width: ${w},
  height: ${h},
  aspect: "${aspect}" as const,
  aspectLabel: "${aspect}",
...
# 把脚手架文件拷到项目根
mkdir -p \
  src/styles src/hooks src/components src/registry src/config \
...
cp "$TEMPLATES/scripts/export-mp4.ts"            scripts/export-mp4.ts
...
cp "$TEMPLATES/src/hooks/useExportTimeline.ts"     src/hooks/useExportTimeline.ts
cp "$TEMPLATES/src/registry/export-visual-min.ts"  src/registry/export-visual-min.ts
...
p.scripts = Object.assign({}, p.scripts, {
  ...
  "export:mp4":         "tsx scripts/export-mp4.ts",
});
...
当前主题：${THEME}（见 .theme）
当前画幅：${ASPECT}（见 .aspect · src/config/stage.ts）
...
音频合成（可选，录制前做）：
  ...
  # 首次在本机：bash $SKILL_DIR/scripts/setup-global-tools.sh
  npm run extract-narrations    # 扫所有章节 narrations.ts → audio-segments.json
  npm run synthesize-audio      # 默认 edge-tts → public/audio/<id>/<step>.mp3
                                # 换 provider：PRESENTATION_TTS=<name> npm run synthesize-audio

导出 MP4（可选，合成口播后）：

  npm run export:mp4            # → output/export.mp4（需全局 ffmpeg + playwright）
                                # 详见 $SKILL_DIR/references/EXPORT.md
