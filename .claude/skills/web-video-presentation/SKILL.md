...
description: 把一篇文章或口播稿，做成"看起来像视频"的点击驱动网页演示（默认 16:9 横屏，支持 9:16 竖屏微信视频号），可选合成口播音频并一键导出 MP4。流程：原始文章 → **一次产出**口播稿 + outline 开发计划 → 用户**一次对齐** 6 件事（稿子 / outline / 画幅 / 主题 / 素材 / 开发模式）→ 网页开发（逐章 / 顺序 / 并行）→ 可选音频合成（**默认 edge-tts 免费中文**，可换 MiniMax / OpenAI / 其它）→ 可选 `npm run export:mp4` 或手动录屏。**outline 只规划节奏与信息密度，不规划动画** —— 动画由章节开发时按 PRINCIPLES + ANTI-AI 法则即时设计。TTS / ffmpeg / Playwright 用 `scripts/setup-global-tools.sh` **一次性全局安装**，各项目不重复装。每次点击推进口播稿的一个节拍，每一步独占整屏，进度条平时隐藏只在悬浮时出现。适用场景：用网页做视频（动态 PPT 但不像 PPT）、把口播稿 / 文章变成可交互的解说、为 B 站 / YouTube / **微信视频号** / 抖音录屏教程、做有电影感的产品 / talk demo。本 Skill 沉淀的是设计方法论 + 协作流程 —— 不绑定任何特定样式 / 字体 / 颜色 —— 因此能复用到任意主题与美学。
---
...
- 想做 "动态 PPT"
- 16:9 横屏或 9:16 竖屏录屏，大字、留白、每屏都要有动效
- 教学 / 产品演示 / keynote 想要电影感
- B 站 / YouTube / **微信视频号** / 抖音视频内容
...
Phase 1   内容编写
   ...
   1.2  一次产出 script.md + outline.md
        （口播稿 + 开发计划）
   ▼
[Checkpoint Plan]      ← 必须停。一次对齐 6 件事：
                         稿子 / outline / 画幅 / 主题 / 素材 / 开发模式
   ▼
Phase 2   网页开发
   2.1  脚手架（按选定主题）
...
└── presentation/       # 脚手架产出的 Vite + React + TS 项目
    ...
    │   ├── export-mp4.ts           # Playwright 录屏 + ffmpeg 合成 MP4
    │   └── tts-providers/          # 每 provider 一个 .sh（内置 edge-tts / minimax / openai）
    │       ├── README.md
    │       ├── edge-tts.sh           # ★ 默认：免费中文、无 API key
    │       ├── minimax.sh
    │       └── openai.sh
...
| Phase 3 音频合成 | `references/AUDIO.md` + `references/SETUP.md`（全局工具） | `templates/scripts/tts-providers/README.md` |
| Phase 4 录屏 / 导出 | `references/RECORDING.md` + `references/EXPORT.md` + `references/PLATFORMS.md` | —— |
...
3. **判断目标画幅**：用户说「视频号 / 抖音 / 竖屏」→ 默认推荐 **9:16**；
   B 站 / YouTube / 横屏 → **16:9**。详见 [`references/PLATFORMS.md`](references/PLATFORMS.md)
...
接下来一次对齐 6 件事：
  ...
  3. 画幅 / 目标平台？
     ★ 推荐：<16:9 横屏 1920×1080 | 或 9:16 竖屏 1080×1920> — 因为 <用户场景>
     16:9 → B 站 / YouTube / 横屏教程
     9:16 → 微信视频号 / 抖音 / 小红书视频（竖屏 feed 主流）
     详见 references/PLATFORMS.md；脚手架 --aspect=9:16 或 --platform=channels
...
- **画幅必须明确**才进入 Phase 2（16:9 或 9:16）。用户说「视频号」未指定横竖 → 默认 9:16，告知用户可改
...
bash <path-to-web-video-presentation>/scripts/scaffold.sh \
  ./presentation \
  --theme=<用户选的主题 id> \
  --aspect=<16:9|9:16>    # 或 --platform=channels（= 9:16 竖屏）
...
要不要合成音频做"自动播放录屏 / 导出 MP4"？
  ✓ 合成 → 扫所有章节的 narrations.ts 出 audio-segments.json，
           调 TTS 合成每步一个 mp3 到 public/audio/。
           **首次在本机**先跑：bash <skill>/scripts/setup-global-tools.sh
           默认 **edge-tts**（免费中文、无 API key）。
           合成后可：
             • npm run export:mp4  — 一键出片（见 references/EXPORT.md）
             • 或浏览器 ?auto=1 手动录屏（见 references/RECORDING.md）
           其它 provider：minimax / openai / 见 scripts/tts-providers/README.md
...
**首次**：`bash <skill>/scripts/setup-global-tools.sh`（edge-tts + ffmpeg + jq + playwright，见 [`references/SETUP.md`](references/SETUP.md)）。
...
npm run synthesize-audio       # 默认 edge-tts，增量
# 换音色：PRESENTATION_TTS_VOICE=zh-CN-XiaoxiaoNeural npm run synthesize-audio -- --force
# 换 provider：PRESENTATION_TTS=minimax npm run synthesize-audio
```

合成完告诉用户：输出位置 / 总段数 / 哪些段时长异常 —— 给最后一次校准节奏的机会。然后进入 Phase 4。
...
| 场景 | 推荐路径 |
|---|---|
| 已合成口播，要最快出片 | **`npm run export:mp4`** → `output/export.mp4`（见 [`references/EXPORT.md`](references/EXPORT.md)） |
| 已合成口播，要最高画质 | Auto 模式 + OBS 录屏（见 [`references/RECORDING.md`](references/RECORDING.md)） |
| 跳过 Phase 3 | Manual 模式手动点击 → 后期配音 |

章节全部完成后，更新 `src/registry/export-visual-min.ts` 里每步动画最小时长，再跑 `export:mp4`。

> agent 在 Phase 3 / Checkpoint Audio 后**主动告诉用户** export:mp4 与手动录屏两条路径。
...
| # | 原则 | 一句话 |
|---|---|---|
| 1 | 固定画幅舞台 | 内容写在 stage.ts 坐标系（16:9→1920×1080；9:16→1080×1920）+ transform scale，没有响应式 |
...
| [`references/AUDIO.md`](references/AUDIO.md) | Phase 3 才读 | 默认 edge-tts + provider 切换 + 故障排查 |
| [`references/SETUP.md`](references/SETUP.md) | Phase 3 / 4 前 | 全局工具一次性安装（edge-tts / ffmpeg / playwright） |
| [`references/EXPORT.md`](references/EXPORT.md) | Phase 4 导出 MP4 | `npm run export:mp4` 流程与音画同步原理 |
| [`templates/scripts/tts-providers/README.md`](templates/scripts/tts-providers/README.md) | 换 / 加 TTS provider 时 | 三函数契约 + 内置 edge-tts / minimax / openai |
| [`references/PLATFORMS.md`](references/PLATFORMS.md) | Checkpoint Plan / Phase 2.1 / Phase 4 | 画幅对照、9:16 竖屏构图与录屏 |
| [`references/RECORDING.md`](references/RECORDING.md) | Phase 4 手动录屏 | OBS / QuickTime + Auto 模式 |
| [`scripts/setup-global-tools.sh`](scripts/setup-global-tools.sh) | 本机首次 | 安装 edge-tts、ffmpeg、playwright chromium |
