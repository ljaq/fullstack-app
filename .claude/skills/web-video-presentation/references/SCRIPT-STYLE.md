...
| **抖音 / 视频号竖屏** | 更燃、更短、更冲击 | ≤ 12 字 | 1 秒钩子 | 高（3 秒/idea） |
| **视频号横屏 16:9** | 同 B 站，略短 | ≤ 18 字 | 2 秒钩子 | 中（4 秒/idea） |
...
> **画幅与口播联动**：指定「视频号 / 抖音」且未说横竖 → 默认 **9:16
> 竖屏**口播（上表「抖音 / 视频号竖屏」列）+ 脚手架 `--aspect=9:16`。
> 明确要横屏 16:9 则用「视频号横屏」列。详见 [`PLATFORMS.md`](PLATFORMS.md)。
...
Checkpoint Plan 让用户一次对齐 6 件事（稿子 / outline / 画幅 / 主题 / 素材
/ 开发模式）。
