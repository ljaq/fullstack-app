...
缺的东西**承认缺** —— 用 placeholder 占位卡（一张写着"image · {画幅}"
描述"的卡片，按真实比例留位 —— 16:9 横屏或 9:16 竖屏，见
`src/config/stage.ts`）。**不要**用 emoji 凑、不要找无关图凑、
...
- **固定画幅舞台**：内容设计在 `src/config/stage.ts` 坐标系上（默认
  16:9→1920×1080；视频号竖屏 9:16→1080×1920），外层 transform scale
  缩到任何视口，外围 letterbox 留黑 —— **没有响应式断点**
- **竖屏 9:16 额外约束**（`orientation === "portrait"`）：纵向堆叠为主、
  禁宽分栏；每步 1~2 个元素；顶底安全区少放关键字 —— 详见
