...
后端是 `scripts/tts-providers/<name>.sh` 一个文件。**默认 provider：edge-tts**
（免费中文、无 API key；需本机一次性安装，见 [`SETUP.md`](SETUP.md)）。

| Provider | 默认 | 何时用 |
|---|---|---|
| `edge-tts` | ✓ | **中文口播首选**；`pip install edge-tts` 或 `setup-global-tools.sh` |
| `minimax` | —— | 需 MiniMax API key；质量稳 |
| `openai`  | —— | 已有 `OPENAI_API_KEY`；英文更好 |
...
PRESENTATION_TTS_VOICE=zh-CN-XiaoxiaoNeural npm run synthesize-audio
```

启动时 runner 会先调 provider 的 `tts_check`。未安装时运行：
...
如果 edge-tts 未装且其它 provider 也没就绪，告诉用户：
...
我可以：
  ...
  4. 用 OpenAI TTS（如果你已有 key）
     export OPENAI_API_KEY=sk-...
     PRESENTATION_TTS=openai npm run synthesize-audio

  5. 换其它 provider — scripts/tts-providers/README.md

  6. 暂时跳过 — 按 audio-segments.json 自己录
```
