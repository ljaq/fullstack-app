...
- **可插拔 TTS**：默认 **edge-tts**（免费中文）；内置 minimax / openai；见 `tts-providers/`
- **一键导出 MP4**：`npm run export:mp4`（Playwright 录屏 + ffmpeg 合成）
- **全局工具一次装**：`scripts/setup-global-tools.sh`（edge-tts / ffmpeg / playwright）
